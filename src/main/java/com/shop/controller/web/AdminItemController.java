...
@Tag(name = "admin-items", description = "관리자 - 상품 등록")
...
public class AdminItemController {
    ...
    @GetMapping(value = {"/list", "list/{page}"}) // url에 페이지 번호가 없는경우, 있는경우
    public String itemManage(ItemSearchDto dto, @PathVariable("page") Optional<Integer> page, Model model) {
        Pageable pageable = PageRequest.of(page.isPresent() ? page.get() : 0, 3);
        Page<Item> items = itemService.getAdminItemPage(dto, pageable);
        model.addAttribute("items", items);
        model.addAttribute("itemSearchDto", dto);
